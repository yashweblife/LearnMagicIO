...
canvasCoordinateSystemDemo.dom.addEventListener(
  "mousemove",
  (e: MouseEvent) => {
    mouse.x = e.clientX - canvasCoordinateSystemDemo.bound.left;
    mouse.y = e.clientY - canvasCoordinateSystemDemo.bound.top;
  }
);
function canvasCoordinateSystemAnimation() {
  ...
  canvasCoordinateSystemDemo.text(
    `X:${mouse.x}, Y:${Math.round(mouse.y)}`,
    mouse,
    10,
    10
  );
